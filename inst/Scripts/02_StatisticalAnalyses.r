  ...
  tryCatch(
    {
      ...
      if(model_type == "binomial") m = glmmTMB(var1 ~ time_to@_in + (1|box) + (1|counter), data = subset(tmp[, var1 := ifelse(var > 0, 1, 0)], sleep == sleep_type & DB == dB_threshold), ziformula = ~0, family = binomial)
