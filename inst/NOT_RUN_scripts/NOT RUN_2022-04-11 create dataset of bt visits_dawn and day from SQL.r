...
dat = subset(dat, as.IDate(in_) == date_ | as.IDate(in_)+1 == date_) #no rows removed
dat[, sleep := ifelse(as.IDate(in_) != date_, 1, 0)]
table(dat$sleep)
#if they don't sleep, remove long time intervals
...
save(dat, file = paste0(getwd(), "/SNB Data/", Sys.Date(), "_data_basis_for_sound_extraction.RData"))
